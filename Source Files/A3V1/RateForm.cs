...
namespace A3V1
{
    public partial class RateForm : Form
    {
        ...
        {
            InitializeComponent();
            userRatingLabel.Text = "" + currentMovie.getUserRating();
            this.trackBar1.ValueChanged += new EventHandler(this.trackBar1Value_Changed);
        }

        private void trackBar1Value_Changed(object sender, EventArgs e)
        {
            userRatingLabel.Text = trackBar1.Value.ToString();
        }

        private void okButton_Click(object sender, EventArgs e)
        {
            short rating = Convert.ToInt16(trackBar1.Value);

            // Save the comment to global data
            GlobalData.selectedMovie.setUserRating(rating);
